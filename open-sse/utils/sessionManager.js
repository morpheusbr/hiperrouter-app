 ...
 * Since hiperrouter is long-running, we simulate this "per-launch" behavior by
 * storing a generated ID in memory for each connection.
 *
 * - If hiperrouter restarts, the ID changes (matching binary restart behavior).
...
const SESSION_HEADER_KEYS = ["x-opencode-session", "x-session-id", "session-id", "session_id", "x-amp-thread-id"];
